...
oralfluid_ps <- subset_samples(ps1, Type == OralFluid)

#Filter low abundance taxa (<10 counts)
oralfluid_ps_filt <- prune_taxa(taxa_sums(oralfluid_ps) > 10, oralfluid_ps)

#Let's filter out OTUs that are not present in at least 20% of our samples

threshold <- 0.2 * nsamples(oralfluid_ps)
oralfluid_ps_3 <- filter_taxa(oralfluid_ps, function(x) sum(x > 0) >= threshold, TRUE)

#agglomerate at genus level

oralfluid_genus <- tax_glom(oralfluid_ps_3, taxrank = "Genus")

taxtab <- as(tax_table(oralfluid_genus), "matrix")

oralfluid_genus_renamed <- renameTaxa(oralfluid_genus, 
                                  pat = "<name>", 
...
set.seed(101010)
oralfluid_genus_net <- netConstruct(oralfluid_genus_renamed,
                                taxRank = "Genus",
                                measure = "sparcc",
                                zeroMethod = "multRepl",
                                normMethod = "clr",
...
set.seed(101010)
props_genus_oralfluid <- netAnalyze(oralfluid_genus_net, clustMethod = "cluster_louvain")

colors <- c("#440154ff", "#404688ff", "#287c8eff", "#27ad81ff", "#8fd744ff")

#Summarize network measures - Degree, betweenness, closeness, eigenvector
set.seed(101010)
summary(props_genus_oralfluid)

#Plot graph
set.seed (10010)
plot(props_genus_oralfluid, 
     nodeColor = "cluster",
     colorVec = colors,
